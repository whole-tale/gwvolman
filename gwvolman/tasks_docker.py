...
from .constants import GIRDER_API_URL, VOLUMES_ROOT, RunStatus
...
class DockerTasks(TasksBase):
    ...
    def recorded_run(self, task, run_id, tale_id, entrypoint):
        ...
        try:
            ...
            self.generate_tro(
                os.path.join(
                    VOLUMES_ROOT, "runs", tale["_id"][0:2], tale["_id"], run["_id"]
                ),
                start_time,
                end_time,
            )
    ...
    def generate_tro(self, path, start, end):
        from .tro import TROHandler

        TROHandler(path, start, end).generate_tro()
